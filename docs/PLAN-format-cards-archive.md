...
### Tâche 1 — Créer le contrat et le validateur de FORMAT CARD
...
- [ ] Écrire les tests `test_valid_card`, `test_missing_required_label`, `test_unknown_taxonomy_value`, `test_rejects_substring_match`, `test_duplicate_card_sections` et `test_upsert_is_idempotent`.
- [ ] Exécuter `python -m unittest discover -s Projects/Sourcing/tests -p "test_*.py" -v` depuis `C:/Users/dbele/Documents/ObsidianVault/Wiki_Claude` et constater les échecs initiaux.
- [ ] Implémenter l'extraction bornée d'une section card jusqu'au prochain titre `##`, au lieu de capturer tout le fichier jusqu'à EOF.
- [ ] Implémenter la validation exacte des trois valeurs : les explications après `—` sont ignorées, les valeurs libres et les sous-chaînes ambiguës sont rejetées.
- [ ] Implémenter `upsert_card()` afin que le second appel retourne `already_present` et ne duplique pas la section ; `replace=True` est le seul mode qui remplace une section existante.
- [ ] Relancer la suite `unittest` et obtenir un résultat vert.

### Tâche 2 — Aligner la spec Sourcing et la skill existante
...
- [ ] Remplacer la description « transcript uniquement » par « transcript verbatim + FORMAT CARD par vidéo lorsque la niche est connue ».
- [ ] Documenter les états `complete` et `needs_review`, les trois labels validés et la règle d'absence de niche.
- [ ] Conserver l'étape 5 d'archivage existante ; préciser qu'après l'écriture l'agent relit le fichier et vérifie qu'il existe exactement une card valide.
- [ ] Préciser qu'une erreur de validation marque le fichier `format_card_status: needs_review` sans modifier le bloc transcript ni masquer l'échec.
- [ ] Documenter que le backfill ne remplace pas les cards existantes sans option explicite.
- [ ] Relire les deux documents et vérifier qu'ils ne contiennent plus deux contrats contradictoires sur le contenu du registre.

### Tâche 3 — Rendre `brief_compiler.py` conscient de la couverture
...
def inspect_cards(videos: list) -> dict:
    """Return n_videos, n_present, n_valid, errors and majority shares."""
...
- [ ] Déplacer les constantes `STYLES` et `MECHANICS` vers `format_card_registry.py`, puis les importer dans `brief_compiler.py` pour éviter deux taxonomies divergentes.
- [ ] Remplacer la regex de card ouverte par l'extracteur borné du module partagé et détecter les sections multiples.
- [ ] Faire retourner `None` à `parse_cards()` dès qu'une vidéo est absente, dupliquée ou invalide ; une card partielle ne doit plus activer la voie noble.
- [ ] Ajouter `inspect_cards()` et un avertissement explicite du type `[niche] n_valid/n_total cards — fallback keywords` lorsque la couverture est incomplète.
- [ ] Ajouter `--strict` à `main()`. En mode strict, lever une erreur lisible avec les chemins des fichiers fautifs ; ne pas faire échouer le mode permissif de développement.
- [ ] Faire accepter à `readiness()` un rapport optionnel sans ajouter de propriété au JSON. Les avertissements voix/moteur restent distincts des avertissements cards.
- [ ] Ajouter le contrôle de majorité minimale de deux tiers en strict ; une niche hétérogène est orientée vers un nouveau cluster/formula, pas forcée dans un vote unique.
- [ ] Remplacer l'injection globale `sys.path.insert(0, ...)` du compilateur par un chargement explicite via `importlib.util.spec_from_file_location` depuis `FCR_MODULE`, afin d'éviter qu'un module homonyme du dossier vault ne masque un import local ou standard-library ; conserver une erreur chaînée et actionnable si le fichier est absent ou non importable.
- [ ] Ajouter une vérification d'import isolée (sans modifier `sys.path`) et confirmer que le compilateur charge bien le fichier `Projects/Sourcing/tools/format_card_registry.py` du vault configuré.
- [ ] Exécuter `python brief_selfcheck.py` et vérifier que `brief.schema.json` n'a pas été modifié.

### Tâche 4 — Corriger la suite de tests du compilateur

**Files:**

- Modify: `test_brief_compiler.py:21-24,63-88`

- [ ] Remplacer l'assertion rigide `len(...) == 5` par des assertions de présence, d'unicité et de cohérence avec le registre réel.
- [ ] Ajouter des fixtures en mémoire pour une niche complète, une niche partielle, une card invalide, une card dupliquée et un libellé qui ne doit pas matcher par sous-chaîne.
- [ ] Tester que la niche partielle utilise le fallback en mode permissif et échoue avec `--strict`.
- [ ] Tester que `readiness(brief)` continue à signaler séparément les valeurs de voix et de moteur non résolues.
- [ ] Utiliser la commande réellement applicable au fichier actuel : `python test_brief_compiler.py`. Ne pas documenter `pytest -k cards` tant que le fichier ne contient pas de fonctions pytest collectables.
- [ ] Exécuter `python test_brief_compiler.py` depuis `C:/Users/dbele/Documents/ObsidianVault/Wiki_Claude/Projects/TikTok Analyzer` et obtenir un résultat vert sur les données présentes.

### Tâche 5 — Produire le worklist de backfill sans automatisation LLM implicite
...
- [ ] Faire de `--check` une opération sans écriture qui liste chaque fichier sans card, chaque card invalide, l'URL et la disponibilité d'un manifest de frames.
- [ ] Faire de `--verify` une opération sans écriture qui retourne un code non nul tant qu'une vidéo attend une card valide.
- [ ] Pour chaque entrée du worklist, lancer `/extract-format` dans le contexte de la niche ; la card est ensuite validée et archivée par `format_card_registry.py`.
- [ ] Ne pas demander à `backfill_cards.py` de « relancer extract-format » : une skill est un workflow agentique, pas une fonction Python appelable implicitement.
- [ ] Ne traiter que les cards absentes durant le backfill initial ; réserver `--replace` à une correction manuelle explicitement revue.
- [ ] Exécuter d'abord les deux `--check`, puis analyser les frames existantes avant de relancer l'API pour les vidéos sans manifest.
- [ ] Après chaque lot, exécuter le `--verify` correspondant et conserver le diff des fichiers transcripts comme preuve de l'absence de modification du verbatim.

### Tâche 6 — Régénérer, vérifier et documenter
...
- Chaque fichier transcript ciblé possède exactement une section `## FORMAT CARD`.
- `python Projects/Sourcing/tools/format_card_registry.py --verify --niche <niche>` ne signale aucune card manquante, dupliquée ou invalide.
...
- `python brief_compiler.py neon_psycho --strict` et `python brief_compiler.py dark_psycho --strict` réussissent après backfill.
...
- La readiness taxonomique est propre ; les valeurs non résolues de voix et de moteur de `dark_psycho` restent explicitement hors périmètre.
