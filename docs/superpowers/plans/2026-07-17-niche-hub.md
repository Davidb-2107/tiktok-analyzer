...
- Repo : dépôt Git Analyzer indépendant ; la branche de travail d’origine était `feat/niche-hub`.
...
if not VAULT.is_dir():
    # Run pytest sur l'hôte : définir VAULT_DIR vers le worktree Vault local.
